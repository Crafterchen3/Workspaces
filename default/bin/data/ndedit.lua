...
multishell.setTitle(multishell.getCount(),sPath)
...
-- Colours
local varColour,highlightColour, keywordColour, commentColour, textColour, bgColour, stringColour
...
local function tryWrite(sLine, regex, colour,var)
    local match = string.match(sLine, regex)
    if var and match then
        match = string.match(match,var)
        if match then
            vars[match:match("%w*")] = true
            _ENV[match:match("%w*")] = true          
        end
...
local function writeVar(sLine)
    local match = string.match(sLine,"%w* =")
    if match then
        return tryWrite(match,"%w*",varColour)
    end
    return nil
end
...
local function writeHighlighted(sLine)
    while #sLine > 0 do
        sLine =
            ...
            tryWrite(sLine, "^%w* =",varColour,"%w* ") or
